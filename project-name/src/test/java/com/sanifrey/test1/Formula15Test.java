...
public class Formula15Test {
	...
	@Test
		 /** 
 * Тестируем метод с входными параметрами 38, 2, 7 и с предполагаемым результатом 24.0 для проверки работы программы с дробными числами.
 ...
 * Предполагается, что программа выполнит следующие расчёты: 38-2*7 и
 * вернёт значение: 24.0.
 */
	public void testFormula1() {
		/**
		 * Добавляем объект для расчётов при помощи метода PublicAddObject со значением ширины "2" и значением длины "7".
		 */
		formula.PublicAddObject( "2", "7");
		/**
		 * Выполняем расчёт свободной площади с выше добавленным объектом при значении всей площади "38".
		 * Метод assertEquals сравнивает значения, если значения будут разные, то будет ошибка. Предполагается,что
		 * вернёт значение: 24.0
		 */
		assertEquals("24.0",formula.PFormula("38"));
		/**
